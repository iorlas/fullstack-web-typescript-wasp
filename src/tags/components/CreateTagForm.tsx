...
export function CreateTagForm({ id, onTagCreated }: CreateTagFormProps) {
  const { handleSubmit, setValue, watch, control, reset } = useForm<CreateTagFormValues>({
    defaultValues: {
      name: "",
  ...
  return (
    <form id={id} onSubmit={stopPropagate(handleSubmit(onSubmit))} className="flex flex-col gap-6">
      <Controller
        name="name"
        control={control}
        rules={{
        ...
        render={({ field, fieldState }) => (
          <Input label="Name" placeholder="Enter tag name" fieldState={fieldState} {...field} />
        )}
      />
